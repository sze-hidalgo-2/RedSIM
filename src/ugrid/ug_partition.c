...
function void ug_partition_rcb(UG_Partition *partition, Arena *arena, UG_Grid *grid, U32 partition_count) {
  ...
  *bounds_local             = range3_f32(v3f_f32(f32_limit_max), v3f_f32(f32_limit_min));
  ...
  log_info("Splitting grid into %u partitions", partition_count);
  Range3_F32 bounds = bounds_global[0];
  ug_partition_rcb_split_recursive(grid, bounds, 0, partition_count, range1_u64(0, grid->elems.len), rcb_key);

  lane_barrier();

  // NOTE(cmat): Assign partition buckets.
  // TODO(cmat): Multi-Core.
  partition->buckets_len = partition_count;
  if (lane_index() == 0) {
    U64 *buckets_at         = arena_push_count(scratch.arena, U64, partition->buckets_len);
    partition->buckets_dat  = arena_push_count(arena, UG_Partition_Bucket, partition->buckets_len);

    // NOTE(cmat): Count how many cells there are in each bucket.
    log_info("Counting buckets");
    for Iter_Index(it, grid->elems.len) {
      partition->buckets_dat[rcb_key[it].partition].cells_len += 1;
    }

    for Iter_Index(it, partition->buckets_len) {
      partition->buckets_dat[it].cells_dat = arena_push_count(arena, U32, partition->buckets_dat[it].cells_len);
    }

    // NOTE(cmat): Fill buckets.
    log_info("Assigning buckets");
    for Iter_Index(it, grid->elems.len) {
      U32 partition_index = rcb_key[it].partition;
      partition->buckets_dat[partition_index].cells_dat[buckets_at[partition_index]++] = rcb_key[it].cell;
    }
  }

  lane_broadcast_type(partition, 0);
